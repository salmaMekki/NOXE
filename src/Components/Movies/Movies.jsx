...
export default function Movies() {
  ...
  async function getTrendingItems(pageNumber) {
    let { data } = await axios.get(
      `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&api_key=955f0c05b120ec6e64af5c7bad730e85&page=${pageNumber}&sort_by=popularity.desc`
    );
  ...
  async function search() {
    let { data } = await axios.get(
      `https://api.themoviedb.org/3/search/movie?api_key=955f0c05b120ec6e64af5c7bad730e85&query=${searchValue}`
    );
            ...
            {movie.map((mo) => (
              <div className="col-md-2" key={mo.id}>
                <div onClick={() => goToDetails(mo.id)} className="movie">
                  ...
                  <h2 className="h6 fw-bold"> {mo.title}</h2>
                </div>
              </div>
            ))}
          </div>
